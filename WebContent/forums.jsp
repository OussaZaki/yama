...
<title>YAMA - You Ask, Moroccans Answer Forums</title>
<link rel="stylesheet" href="/YamaWeb/css/bootstrap.min.css">
<style>
body {
	padding-top: 60px;
...
<div class="navbar navbar-inverse navbar-fixed-top">
        <div class="navbar-inner">
            <div class="container">
                <a href="/YamaWeb/Profile/cloud" class="brand">YAMA</a>
                <div class="nav-collapse">
                    <ul class="nav">
                        <li><a href="/YamaWeb/Profile">Home</a></li>
                        <li class="active"><a href="/YamaWeb/Forum">Forum</a></li>
                    </ul>
                    ...
                    <ul class="nav pull-right">
                        <li><a href="/YamaWeb/Profile/edit">Account</a></li>
                        <li><a href="/YamaWeb/User/logout">Logout</a></li>
                    </ul>
                </div>
	...
	<div class="container">
		...
		<ul class="breadcrumb">
			<li><i class="icon-home"></i><a
				href="/YamaWeb/Forum"> Board Index</a></li>
				<%for(int i = parents.length - 1; i > 0; i--){ %>
				<li><span class="divider">/</span><a href=<%= "/YamaWeb/Forum/index/" + parents[i].getId()%>> 
		...
		<div style="margin-left: 8px">
			<h4>
				<a href="<%= "/YamaWeb/Forum/index/" + category.getId()%>"><%= category.getName() %></a>
			</h4>
		</div>
		<% if(subForums.size() != 1) {%>
		<table class="table table-hover table-bordered">
			...
			<tr>
				<td>
					<div><a href="<%= "/YamaWeb/Forum/index/" + forum.getId()%>"><strong><%= forum.getName() %></strong></a></div>
					...
					%>
						<div>
							...
							<% for(int i = 0; i < subForumsObjects.length; i++) { %> 
								<a href="<%= "/YamaWeb/Forum/index/" + subForumsObjects[i].getId()%>"><%= subForumsObjects[i].getName() %></a><%= (i == subForumsObjects.length - 1) ? "" : "," %></a>
							<% } %>
						</div>
					<% } %>
				</td>
				...
				<% ForumPostTable p = posts.get(forum.getLastPostId());
					...
					<div><small><a href="/YamaWeb/Profile/details/<%=u.getId()%>"><%=u.getName() %></a></small> 
					<a href="/YamaWeb/Forum/topic/<%= p.getTopicId()%>"><i class="icon-play-circle" title="View the latest post"></i></a></div>
	...
	<p><a class="btn btn-success" href=<%= "/YamaWeb/Forum/newtopic/" + category.getId()%>><i class="icon-plus-sign icon-white"></i> New Topic</a></p>
	<table class="table table-hover table-bordered">
		<thead>
			<tr class="well">
				<th>Topic</th>
		...
		<%for(ForumTopicTable topic: topics){ %>
			<tr>
				<td>
					<div><a href="/YamaWeb/Forum/topic/<%=topic.getId()%>"><%= topic.getTitle() %></a></div>
	...
	<script type="text/javascript" src="/YamaWeb/js/bootstrap.js"></script>
</body>
</html>